...
class PostgresDigitizerLogger(PostgresSensorLogger):
    ...
    '''
    def __init__(self, integrated_power_endpoint, auths_file = None, **kwargs):
        '''
        '''
        PostgresSensorLogger.__init__(self,**kwargs)
        self._auths_file = auths_file
        self.python_interface = Interface(dripline_config={'auth-file': self._auths_file})
    ...
    def process_payload(self, a_payload, a_routing_key_data, a_message_timestamp):
        ...
        if a_routing_key_data == 'fast_daq.medium_spectrum':
            ...
            self.python_interface.set(integrated_power_endpoint, calculated_integrated_power)
            self.python_interface.cdm(integrated_power_endpoint, "scheduled_log")
        # do the insert
